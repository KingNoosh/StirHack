...
use \Exception;
use \PDO;
use App\Model\Log as ChildLog;
use App\Model\LogQuery as ChildLogQuery;
use App\Model\Map\LogTableMap;
 ...
 * @method     ChildLogQuery orderByTimestamp($order = Criteria::ASC) Order by the timestamp column
 ...
 * @method     ChildLogQuery groupByTimestamp() Group by the timestamp column
 ...
 * @method     ChildLog findOneByTimestamp(int $timestamp) Return the first ChildLog filtered by the timestamp column
 ...
 * @method     ChildLog requireOneByTimestamp(int $timestamp) Return the first ChildLog filtered by the timestamp column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildLog[]|ObjectCollection findByTimestamp(int $timestamp) Return ChildLog objects filtered by the timestamp column
...
abstract class LogQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \App\Model\Base\LogQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\App\\Model\\Log', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, timestamp, data FROM log WHERE id = :p0';
        try {
    ...
    public function filterByTimestamp($timestamp = null, $comparison = null)
    {
        if (is_array($timestamp)) {
            ...
            if (isset($timestamp['min'])) {
                $this->addUsingAlias(LogTableMap::COL_TIMESTAMP, $timestamp['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($timestamp['max'])) {
                $this->addUsingAlias(LogTableMap::COL_TIMESTAMP, $timestamp['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(LogTableMap::COL_TIMESTAMP, $timestamp, $comparison);
    }
